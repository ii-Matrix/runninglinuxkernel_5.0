...
#define DEMO_NAME "my_demo_error"
...
static int demodrv_open(struct inode *inode, struct file *file)
{
    ...
    ret = kfifo_alloc(&data->mydemo_kfifo,MYDEMO_MAX_DEVICES,GFP_KERNEL);
    if(ret) {
        kfree(data);
        return -ENOMEM;
    }
...
demodrv_read(struct file *file, char __user *buf, size_t lbuf, loff_t *ppos)
{
    ...
    if(kfifo_is_empty(&data->mydemo_kfifo)) {
        if(file->f_flags & O_NONBLOCK)
            return -EAGAIN;
        
        printk("%s,pid = %d,going to sleep\n",__func__,current->pid);
        ret = wait_event_interruptible(data->read_queue,!kfifo_is_empty(&data->mydemo_kfifo));
        if(ret)
            return EIO;
    }
   ...
   ret = kfifo_to_user(&data->mydemo_kfifo,buf,lbuf,&actual_readed);

    if(ret)
        return -EIO;
    
    if(!kfifo_is_full(&data->mydemo_kfifo))
...
demodrv_write(struct file *file, const char __user *buf, size_t count, loff_t *f_ops)
{
    ...
    if(kfifo_is_full(&data->mydemo_kfifo)) {
        ...
        printk("%s,pid = %d,going to sleep\n",__func__,current->pid);
        ret = wait_event_interruptible(data->write_queue,!kfifo_is_full(&data->mydemo_kfifo));
        if(ret)
            return ret;
       }

    // ret = copy_from_user(device_buffer + *f_ops, buf, need_write);
    ret = kfifo_from_user(&data->mydemo_kfifo,buf,count,&actual_write);
    if(ret)
        return -EIO;
