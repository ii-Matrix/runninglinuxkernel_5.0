...
#define DEMO_DEV_NAME "my_demo_drv"
...
static ssize_t
demodrv_read_write(void *buf,size_t len,int rw)
{
    int ret,npages,i;
    struct page **pages;
...
demodrv_write(struct file *file,const char __user *buf,size_t count,loff_t *ppos)
{
    ...
    printk("%s:write nbyte = %d done at ppos = %d\n",
        __func__,nbyte,(int)*ppos);
...
demodrv_mmap(struct file *filp,struct vm_area_struct *vma)
{
    ...
    printk("%s:mmaping %ld bytes of device buffer at offset &ld\n",
            __func__,len,offset);
